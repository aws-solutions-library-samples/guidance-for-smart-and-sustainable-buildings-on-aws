...
このガイダンスのサンプルコードを実行する際のAWSサービスの費用はお客様の負担となります。2024年8月時点で、us-west-2 (オレゴン) リージョンでこのソリューションを実行する場合の推定コストは、月額約$233.16 (USD)です。

注意: 実際のコストは、デバイス数、Amazon Managed Grafanaのユーザー数、Amazon Timestreamのクエリ頻度などのによって異なる場合があります。
...
| Amazon Timestream               | 1KBレコードサイズ、0.62 GBメモリストア書き込み、32 TCU x 0.016時間 x 730時間、3日間のメモリ保持、3ヶ月の磁気ストレージ保持 | $195.5           |
| Amazon Managed Grafana          | 2 エディタ、3 ビューワー                                                                                                   | $33              |
